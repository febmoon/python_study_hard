...
# 정보를 제공하지 않는 방법으로 주석처리가 되어 있는 부분은
# 파이썬 인터프리터가 코드로 인식하지 않음.
...
'''
19-20번 라인의 출력 결과가 동일하기 때문에 큰 따옴표의 필요성에 대해 
...
""내에 작성을 해야겠네요.
'''
print("제 이름은 조수진입니다. 나이는 26살이고, 부산광역시 사하구에 삽니다.")
print("MBTI는 ISFJ입니다. "
      "엔터를 치니까 뭐가 추가됐네요. ") # 엔터치더라도 스페이스바로 띄어쓰기 해줘야 한다
print('''
이런 경우에는 엔터를 치게 됐을 경우에
콘솔에서 엔터키가 적용이 된 상태로
작성이 가능합니다.
''')    # 다중주석은 아까 처리안됐는데  print 넣으니까 출력된다~~
'''
...
'''
# print("제 이름은 조수진입니다. 나이는 26살이고, 부산광역시 사하구에 삽니다.")
introduction = "제 이름은 조수진입니다. 나이는 26살이고, 부산광역시 사하구에 삽니다."
a = "제 이름은 조수진입니다. 나이는 26살이고, 부산광역시 사하구에 삽니다."
print(a)

print(type(introduction))
'''
변수(variable) : 데이터를 저장하는 바구니 -> 이름을 어떻게 붙여주는가는 자유롭습니다.
                a도 가능/  introduction도 가능
                
                변수 명명 규칙(python 버전으로)
                    1. 변수의 경우 소문자로만 입력한다.
                    2. 여러 단어가 합쳐진 변수의 경우 _로 연결한다.
                        ex) school_name -> 이런 것을 snake case라고 합니다.
...
print(b)    # 결과값 : 1
b = b+1
print(b)    # 결과값 : 2 -> 코드는 순서대로 실행됩니다.
'''
대입연산자
1. = : = 오른쪽에 있는 데이터를 = 왼쪽에 있는 변수에 대입한다는 의미.
        즉, 94번 라인의 b=1이라는 1이라는 정수 데이터를 b라는 변수에 대입하고,
        96번 라인에서 b(1) + 1라는 데이터를 다시 b라는 변수에 대입했다는 의미로
        그 결과 97번 라인에서 2가 출력됨. 파이썬에서 등호는 같다는 의미가 아니다! 변수=변수에 넣을 데이터
...
""내에 변수를 불러오기 위해서는 중괄호({})를 삽입하고, 불러오고자 한느 변수를 기입함.
'''
...
이를 해결하기 위해서 우리가 input()을 이용할 수 있습니다.
 '''
# input("당신의 이름이 무엇입니까? >>>")
'''
...
즉, 데이터를 입력받기는 했지만 변수에 저장하는 과정이 없었기 때문에 데이터가 휘발됩니ㅏ.
