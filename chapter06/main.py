'''
1. 함수(fucntion) : 특정 작업을 수행하는 코드 블록을 정의하는 방법
...
라고 볼 수 있습니다. 그런데 컴퓨터는 시키는 대로만 하기 때문에 사진을 찍기 위해서
...
ex) reeborg's world의 turn_right()
...
3. 함수 용어 정리(멘토 파이썬 교재에 있습니다.)
    ...
    2) 인자/특질/인수(argument) : 함수에 전달할 입력값(input)
...
# #  함수 정의
# def write_name(name):  # 정의할 때 소괄호 내에 있는 것을 parameter라고 합니다. new*
#     print(f"당신의 이름은 {name}입니다.")
#
# #  함수 호출
# write_name("조수진") #호출할 때 소괄호 내에 있는 것을 argument  라고 합니다.
#
# def write_name_age(name,age): # parameter가 복수인 사례 및 함수 정의
#     print(f"당신의 이름은 {name}이고, 나이는 {age}살입니다.")
#
# write_name_age("조수진",26) #매개변수가 두 개이기 때문에 argument도 두 개인 상황
# write_name_age(age=26, name="조수진")  # keyword argument라고 합니다.
...
파이써너 내장 함수는 이미 함수가 정의돼있고, 개발자들은 함수 호출만 잘 하면 됩니다.
사용자 함수는 개발자 자신이 함수를 정의하고, 그 후에 호출하는 것까지의 과정이라고 생각하시면 됩니다.

내장 함수 예시
print() / type() / int() / float() / input() - string 함수 index
...
# call2("춥3다")
#
# # call3() # 이경우 return 출력안됨
#
# # print(call3()) # 이래야 return 출력됨 -> 불편하지 않나?
# print(call3() +1)
#
...
#     return age+1
#
# # this_year_age = int(input("올해 나이를 입력하세요 >>> "))
# print(call4_2(this_year_age))



'''
call3()/ call4() 유형에서 함수 내에 print()를 집어넣어두면 main 단계에서 (들여쓰기가 되어있지 않은 단계에서)
print() 함수를 입력할 필요가 없어 훨씬 편할 것 같은데
왜 굳이  return 형태로 입력해서 main에서 일일이 print() 함수를 적용해야 하는가?

함수형 프로그래밍(functional programming) : 특정한 ㅎ마수1의 결과값이
    또 다른 함수2의 argument로 사용되는 것을 의미합니다.
        그리고 함수2의 결과값이 함수3의 argument로 사용되는 것이 반복된다면,
'''
# 사용자 함수를 정의
# def introduce(name, address):  # call4(d).유형으로 정의 new*
#     return f"제 이름은 {name}이고, {address}에 삽니다."
# # 함수1 사용 : input() -> 파이썬 내장 함수
# name = input("이름을 입력하세요>>>")
# address = input("주소를 입력하세요>>>")
#
# #함수1의 결과값을 함수2인 사용자 함수argument로 사용 -> 그 결과값을 함수3인 print()함수의 argument로 사용
...
# my_money = 3000
# drink_price = 700
# for i in range(my_money//drink_price + 1):  # int(my_money/drink_price)
#     print(f"음료수 = {i}개, 잔돈 = {my_money-(drink_price*i)}")

# return 타입 없으니까  print()로 마무리 지어놨습니다 / 매개변수명이 통제돼있으므로  vending_machine(money)형태로 작성
# def vending_machine(money):
#     for i in range(money // 700 + 1):  # int(my_money/drink_price)
#         print(f"음료수 = {i}개, 잔돈 = {money - (700 * i)}")
#
# vending_machine(9000)
'''
예제 : 구구단 출려하기
...
'''
#call2
# dan = int(input("몇 단을 출력하시겠습니까?>>>")) # 인풋할 때 인트 꼭 적기!!
# def multiply(n):
#     for i in range(1,10,1): # 이렇게 말고 for number in range(1,10,1) # (시작값, 종료값,  증감값)
#         print(f"{n} X {i} = {n*i}")
# call1
def multiply():
