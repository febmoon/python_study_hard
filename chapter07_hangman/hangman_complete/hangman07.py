...
'''
import 문 학습
    import문을 단독으로만 쓸 경우에는 모듈명, 변수명 혹은 모듈명.메서드명()을 명시해야 함.
    모듈명. 변수명의 예시 -> hangman_arts.log
    모듈명.메서드()의 예시-> random.choice()
import문의 응용
    ...
    *(asterisk) : all의 의미. 
    ...
    특정 변수가 해당파일에서 자체적으로 생성됐는지,
    혹은 다른 어떤 파일들에서 참조되었는지를 가시적으로 확인할 수 없다는 점 때문에
    제한적으로 사용됨.
    
    from a import a,b,c의 경우는 그나마 낫지만
    *개념과 합쳐졌을 때는 가독성이 매우 떨어진다는 ㄴ단점
    
    마찬가지로, 
    from randome import choice의 경우에도 해당 메서드 하나만 참조했기 때문에
    별 문제 없지만
    from random import*로 작성했을 경우
    choice가 자체적으로 정의되 ㄴ메서드이니지 혹은 다른데서 가지고 온 변수인지 메서드인지
